...
*  version.
*
* Bu eklenti CSmiLeFaCe tarafýndan son hale getirilmiþ ve www.csplugin.com 'da paylaþýlmýþtýr.
* Bana, yani CSmiLeFaCe'e ulaþmak istiyorsanýz ->> https://www.facebook.com/6.Canavar | http://steamcommunity.com/id/6Canavar/
* Yürü be Atlas E-Spor takýmý!

Bildiğimiz stats eklentisi (top15), fakat bunu baştan aşağı değiştirdim. Mesela artık top15 yok onun yerine top10,top20,top30...top100 var ve topme var. Görsellik açısından ise baya şekilli şukullu duruyor. Düzenlemeniz içinde, içinde psd dosyası mevcuttur.

Uzun süredir kendi serverım için düzenlediğim ve kullandığım bir eklentidir, serverı kapattığım için ve bazı arkadaşlara sözüm olduğu için paylaşıyorum.

Read more: http://www.csplugin.com/2015/12/csm-statsx.html#ixzz5pDk8XhU2
...
.topme /topme !topme
.hp /hp !hp
.statsme /statsme !statsme
.rankstats /rankstats !rankstats
.me /me !me
.rank /rank !rank
.stats /stats !stats
...
{
	// Register plugin.
	register_plugin("StatsX", AMXX_VERSION_STR, "AMXX Dev Team [Edit CSmiLeFaCe]")
	...
	authidgoster = register_cvar("rank_authidgoster","0")
	...
	register_clcmd("say /top10", "cmdTop15", 0, "- display top 15 players (MOTD)")
	register_clcmd("say !top10", "cmdTop15", 0, "- display top 15 players (MOTD)")
	register_clcmd("say .top10", "cmdTop15", 0, "- display top 15 players (MOTD)")

	register_clcmd("say .top20", "cmdTop1020", 0)
	register_clcmd("say !top20", "cmdTop1020", 0)
	register_clcmd("say /top20", "cmdTop1020", 0)

	register_clcmd("say /top30", "cmdTop2030", 0)
	register_clcmd("say !top30", "cmdTop2030", 0)
	register_clcmd("say .top30", "cmdTop2030", 0)

	register_clcmd("say /top40", "cmdTop3040", 0)
	register_clcmd("say !top40", "cmdTop3040", 0)
	register_clcmd("say .top40", "cmdTop3040", 0)

	register_clcmd("say /top50", "cmdTop4050", 0)
	register_clcmd("say !top50", "cmdTop4050", 0)
	register_clcmd("say .top50", "cmdTop4050", 0)

	register_clcmd("say /top60", "cmdTop5060", 0)
	register_clcmd("say !top60", "cmdTop5060", 0)
	register_clcmd("say .top60", "cmdTop5060", 0)

	register_clcmd("say /top70", "cmdTop6070", 0)
	register_clcmd("say !top70", "cmdTop6070", 0)
	register_clcmd("say .top70", "cmdTop6070", 0)

	register_clcmd("say /top80", "cmdTop7080", 0)
	register_clcmd("say !top80", "cmdTop7080", 0)
	register_clcmd("say .top80", "cmdTop7080", 0)

	register_clcmd("say /top90", "cmdTop8090", 0)
	register_clcmd("say !top90", "cmdTop8090", 0)
	register_clcmd("say .top90", "cmdTop8090", 0)

	register_clcmd("say /top100", "cmdTop90100", 0)
	register_clcmd("say !top100", "cmdTop90100", 0)
	register_clcmd("say .top100", "cmdTop90100", 0)

	register_clcmd("say !topme", "siralamadakiyerim", 0)
	register_clcmd("say .topme", "siralamadakiyerim", 0)
	register_clcmd("say /topme", "siralamadakiyerim", 0)
...
format_top15(sBuffer[MAX_BUFFER_LENGTH + 1])
{
	...
	if (iMax > 10)
		iMax = 10
	new lNick[20], lKills[20], lDeaths[20], lHs[20], lAcc[20]
	format(lNick, 19, "Nick (AuthID)")
	format(lKills, 19, "Ã–ldÃ¼rme")
	format(lDeaths, 19, "Ã–lme")
	format(lHs, 19, "Kafadan Ã–ldÃ¼rme")
	format(lAcc, 19, "Isabet Orani")

	ucfirst(lAcc)

	iLen = format( sBuffer, MAX_BUFFER_LENGTH,
                  "<head><META http-equiv=Content-Type content='text/html ;charset=UTF-8'></head><style>body{background:#191818;background-image: url(^"http://i.hizliresim.com/d59r4p.jpg^");margin:5pt;padding:2pt;font-family:Tahoma;color:#ccc}.A{background-color:#262626;opacity:0.6;filter:alpha(opacity=60);}.B{background-color:#0b0b0b;opacity:0.6;filter:alpha(opacity=60);}td{font-size:14px}</style><center><table width=600 background=^"http://i.hizliresim.com/1Db8qp.png^">" )
	...
	for (new i = 0; i < iMax && MAX_BUFFER_LENGTH - iLen > 0; i++)
	{
		...
		if(get_pcvar_num(authidgoster) == 1){
			iLen += format(sBuffer[iLen], MAX_BUFFER_LENGTH - iLen, "<tr class=%s><td>%d<td>%s (%s)<td>%d<td>%d<td>%d<td>%0.2f%%",istate, i + 1, t_sName, sAuthid, izStats[STATS_KILLS],
...
format_top1020(sBuffer[MAX_BUFFER_LENGTH + 1])
{
	new iMax = get_statsnum()
	new izStats[8], izBody[8], istate[4]
	new iLen = 0
	...
	iLen = format( sBuffer, MAX_BUFFER_LENGTH,
                  "<head><META http-equiv=Content-Type content='text/html ;charset=UTF-8'></head><style>body{background:#191818;background-image: url(^"http://i.hizliresim.com/d59r4p.jpg^");margin:5pt;padding:2pt;font-family:Tahoma;color:#ccc}.A{background-color:#262626;opacity:0.6;filter:alpha(opacity=60);}.B{background-color:#0b0b0b;opacity:0.6;filter:alpha(opacity=60);}td{font-size:14px}</style><center><table width=600 background=^"http://i.hizliresim.com/ZQ68mA.png^">" )
	iLen += format( sBuffer[iLen], MAX_BUFFER_LENGTH - iLen,
                  "<tr><td><br/> <td></tr><tr><td><br/> <td></tr><tr><td><br/> <td></tr><tr><td><br/> <td></tr><tr><td><br/> <b><td></tr><tr><td>%s<td>%s<td>%s<td>%s<td>%s<td>%s</b>",
                  "#", lNick, lKills, lDeaths, lHs, lAcc )

	for (new i = 10; i < iMax && MAX_BUFFER_LENGTH - iLen > 10; i++)
	{
		if (equal(istate,"A")) copy(istate,3,"B")
                else copy(istate,3,"A")
                get_stats(i, izStats, izBody, t_sName, MAX_NAME_LENGTH,sAuthid,MAX_NAME_LENGTH)
...
format_top2030(sBuffer[MAX_BUFFER_LENGTH + 1])
{
	new iMax = get_statsnum()
	new izStats[8], izBody[8], istate[4]
	new iLen = 0
	...
	iLen = format( sBuffer, MAX_BUFFER_LENGTH,
                  "<head><META http-equiv=Content-Type content='text/html ;charset=UTF-8'></head><style>body{background:#191818;background-image: url(^"http://i.hizliresim.com/d59r4p.jpg^");margin:5pt;padding:2pt;font-family:Tahoma;color:#ccc}.A{background-color:#262626;opacity:0.6;filter:alpha(opacity=60);}.B{background-color:#0b0b0b;opacity:0.6;filter:alpha(opacity=60);}td{font-size:14px}</style><center><table width=600 background=^"http://i.hizliresim.com/VXOkmV.png^">" )
	iLen += format( sBuffer[iLen], MAX_BUFFER_LENGTH - iLen,
                  "<tr><td><br/> <td></tr><tr><td><br/> <td></tr><tr><td><br/> <td></tr><tr><td><br/> <td></tr><tr><td><br/> <b><td></tr><tr><td>%s<td>%s<td>%s<td>%s<td>%s<td>%s</b>",
                  "#", lNick, lKills, lDeaths, lHs, lAcc )

	for (new i = 20; i < iMax && MAX_BUFFER_LENGTH - iLen > 20; i++)
	{
		if (equal(istate,"A")) copy(istate,3,"B")
                else copy(istate,3,"A")
...
format_top3040(sBuffer[MAX_BUFFER_LENGTH + 1])
{
	new iMax = get_statsnum()
	new izStats[8], izBody[8], istate[4]
	new iLen = 0
	...
	iLen = format( sBuffer, MAX_BUFFER_LENGTH,
                  "<head><META http-equiv=Content-Type content='text/html ;charset=UTF-8'></head><style>body{background:#191818;background-image: url(^"http://i.hizliresim.com/d59r4p.jpg^");margin:5pt;padding:2pt;font-family:Tahoma;color:#ccc}.A{background-color:#262626;opacity:0.6;filter:alpha(opacity=60);}.B{background-color:#0b0b0b;opacity:0.6;filter:alpha(opacity=60);}td{font-size:14px}</style><center><table width=600 background=^"http://i.hizliresim.com/nmdXZg.png^">" )
	iLen += format( sBuffer[iLen], MAX_BUFFER_LENGTH - iLen,
                  "<tr><td><br/> <td></tr><tr><td><br/> <td></tr><tr><td><br/> <td></tr><tr><td><br/> <td></tr><tr><td><br/> <b><td></tr><tr><td>%s<td>%s<td>%s<td>%s<td>%s<td>%s</b>",
                  "#", lNick, lKills, lDeaths, lHs, lAcc )

	for (new i = 30; i < iMax && MAX_BUFFER_LENGTH - iLen > 30; i++)
	{
		if (equal(istate,"A")) copy(istate,3,"B")
                else copy(istate,3,"A")
                get_stats(i, izStats, izBody, t_sName, MAX_NAME_LENGTH)
...
format_top4050(sBuffer[MAX_BUFFER_LENGTH + 1])
{
	new iMax = get_statsnum()
	new izStats[8], izBody[8], istate[4]
	new iLen = 0
	...
	iLen = format( sBuffer, MAX_BUFFER_LENGTH,
                  "<head><META http-equiv=Content-Type content='text/html ;charset=UTF-8'></head><style>body{background:#191818;background-image: url(^"http://i.hizliresim.com/d59r4p.jpg^");margin:5pt;padding:2pt;font-family:Tahoma;color:#ccc}.A{background-color:#262626;opacity:0.6;filter:alpha(opacity=60);}.B{background-color:#0b0b0b;opacity:0.6;filter:alpha(opacity=60);}td{font-size:14px}</style><center><table width=600 background=^"http://i.hizliresim.com/LgMBm1.png^">" )
	iLen += format( sBuffer[iLen], MAX_BUFFER_LENGTH - iLen,
                  "<tr><td><br/> <td></tr><tr><td><br/> <td></tr><tr><td><br/> <td></tr><tr><td><br/> <td></tr><tr><td><br/> <b><td></tr><tr><td>%s<td>%s<td>%s<td>%s<td>%s<td>%s</b>",
                  "#", lNick, lKills, lDeaths, lHs, lAcc )

	for (new i = 40; i < iMax && MAX_BUFFER_LENGTH - iLen > 40; i++)
	{
		if (equal(istate,"A")) copy(istate,3,"B")
                else copy(istate,3,"A")
                get_stats(i, izStats, izBody, t_sName, MAX_NAME_LENGTH)
...
format_top5060(sBuffer[MAX_BUFFER_LENGTH + 1])
{
	new iMax = get_statsnum()
	new izStats[8], izBody[8], istate[4]
	new iLen = 0
	...
	iLen = format( sBuffer, MAX_BUFFER_LENGTH,
                  "<head><META http-equiv=Content-Type content='text/html ;charset=UTF-8'></head><style>body{background:#191818;background-image: url(^"http://i.hizliresim.com/d59r4p.jpg^");margin:5pt;padding:2pt;font-family:Tahoma;color:#ccc}.A{background-color:#262626;opacity:0.6;filter:alpha(opacity=60);}.B{background-color:#0b0b0b;opacity:0.6;filter:alpha(opacity=60);}td{font-size:14px}</style><center><table width=600 background=^"http://i.hizliresim.com/7XJl5v.png^">" )
	iLen += format( sBuffer[iLen], MAX_BUFFER_LENGTH - iLen,
                  "<tr><td><br/> <td></tr><tr><td><br/> <td></tr><tr><td><br/> <td></tr><tr><td><br/> <td></tr><tr><td><br/> <b><td></tr><tr><td>%s<td>%s<td>%s<td>%s<td>%s<td>%s</b>",
                  "#", lNick, lKills, lDeaths, lHs, lAcc )

	for (new i = 50; i < iMax && MAX_BUFFER_LENGTH - iLen > 50; i++)
	{
		if (equal(istate,"A")) copy(istate,3,"B")
                else copy(istate,3,"A")
                get_stats(i, izStats, izBody, t_sName, MAX_NAME_LENGTH)
...
format_top6070(sBuffer[MAX_BUFFER_LENGTH + 1])
{
	new iMax = get_statsnum()
	new izStats[8], izBody[8], istate[4]
	new iLen = 0
	...
	iLen = format( sBuffer, MAX_BUFFER_LENGTH,
                  "<head><META http-equiv=Content-Type content='text/html ;charset=UTF-8'></head><style>body{background:#191818;background-image: url(^"http://i.hizliresim.com/d59r4p.jpg^");margin:5pt;padding:2pt;font-family:Tahoma;color:#ccc}.A{background-color:#262626;opacity:0.6;filter:alpha(opacity=60);}.B{background-color:#0b0b0b;opacity:0.6;filter:alpha(opacity=60);}td{font-size:14px}</style><center><table width=600 background=^"http://i.hizliresim.com/gVg7E2.png^">" )
	iLen += format( sBuffer[iLen], MAX_BUFFER_LENGTH - iLen,
                  "<tr><td><br/> <td></tr><tr><td><br/> <td></tr><tr><td><br/> <td></tr><tr><td><br/> <td></tr><tr><td><br/> <b><td></tr><tr><td>%s<td>%s<td>%s<td>%s<td>%s<td>%s</b>",
                  "#", lNick, lKills, lDeaths, lHs, lAcc )

	for (new i = 60; i < iMax && MAX_BUFFER_LENGTH - iLen > 60; i++)
	{
		if (equal(istate,"A")) copy(istate,3,"B")
                else copy(istate,3,"A")
                get_stats(i, izStats, izBody, t_sName, MAX_NAME_LENGTH)
...
format_top7080(sBuffer[MAX_BUFFER_LENGTH + 1])
{
	new iMax = get_statsnum()
	new izStats[8], izBody[8], istate[4]
	new iLen = 0
	...
	iLen = format( sBuffer, MAX_BUFFER_LENGTH,
                  "<head><META http-equiv=Content-Type content='text/html ;charset=UTF-8'></head><style>body{background:#191818;background-image: url(^"http://i.hizliresim.com/d59r4p.jpg^");margin:5pt;padding:2pt;font-family:Tahoma;color:#ccc}.A{background-color:#262626;opacity:0.6;filter:alpha(opacity=60);}.B{background-color:#0b0b0b;opacity:0.6;filter:alpha(opacity=60);}td{font-size:14px}</style><center><table width=600 background=^"http://i.hizliresim.com/ZQ68mZ.png^">" )
	iLen += format( sBuffer[iLen], MAX_BUFFER_LENGTH - iLen,
                  "<tr><td><br/> <td></tr><tr><td><br/> <td></tr><tr><td><br/> <td></tr><tr><td><br/> <td></tr><tr><td><br/> <b><td></tr><tr><td>%s<td>%s<td>%s<td>%s<td>%s<td>%s</b>",
                  "#", lNick, lKills, lDeaths, lHs, lAcc )

	for (new i = 70; i < iMax && MAX_BUFFER_LENGTH - iLen > 70; i++)
	{
		if (equal(istate,"A")) copy(istate,3,"B")
                else copy(istate,3,"A")
                get_stats(i, izStats, izBody, t_sName, MAX_NAME_LENGTH)
...
format_top8090(sBuffer[MAX_BUFFER_LENGTH + 1])
{
	new iMax = get_statsnum()
	new izStats[8], izBody[8], istate[4]
	new iLen = 0
	...
	iLen = format( sBuffer, MAX_BUFFER_LENGTH,
                  "<head><META http-equiv=Content-Type content='text/html ;charset=UTF-8'></head><style>body{background:#191818;background-image: url(^"http://i.hizliresim.com/d59r4p.jpg^");margin:5pt;padding:2pt;font-family:Tahoma;color:#ccc}.A{background-color:#262626;opacity:0.6;filter:alpha(opacity=60);}.B{background-color:#0b0b0b;opacity:0.6;filter:alpha(opacity=60);}td{font-size:14px}</style><center><table width=600 background=^"http://i.hizliresim.com/o85bok.png^">" )
	iLen += format( sBuffer[iLen], MAX_BUFFER_LENGTH - iLen,
                  "<tr><td><br/> <td></tr><tr><td><br/> <td></tr><tr><td><br/> <td></tr><tr><td><br/> <td></tr><tr><td><br/> <b><td></tr><tr><td>%s<td>%s<td>%s<td>%s<td>%s<td>%s</b>",
                  "#", lNick, lKills, lDeaths, lHs, lAcc )

	for (new i = 80; i < iMax && MAX_BUFFER_LENGTH - iLen > 80; i++)
	{
		if (equal(istate,"A")) copy(istate,3,"B")
                else copy(istate,3,"A")
                get_stats(i, izStats, izBody, t_sName, MAX_NAME_LENGTH)
...
format_top90100(sBuffer[MAX_BUFFER_LENGTH + 1])
{
	new iMax = get_statsnum()
	new izStats[8], izBody[8], istate[4]
	new iLen = 0
	...
	iLen = format( sBuffer, MAX_BUFFER_LENGTH,
                  "<head><META http-equiv=Content-Type content='text/html ;charset=UTF-8'></head><style>body{background:#191818;background-image: url(^"http://i.hizliresim.com/d59r4p.jpg^");margin:5pt;padding:2pt;font-family:Tahoma;color:#ccc}.A{background-color:#262626;opacity:0.6;filter:alpha(opacity=60);}.B{background-color:#0b0b0b;opacity:0.6;filter:alpha(opacity=60);}td{font-size:14px}</style><center><table width=600 background=^"http://i.hizliresim.com/lrXMmb.png^">" )
	iLen += format( sBuffer[iLen], MAX_BUFFER_LENGTH - iLen,
                  "<tr><td><br/> <td></tr><tr><td><br/> <td></tr><tr><td><br/> <td></tr><tr><td><br/> <td></tr><tr><td><br/> <b><td></tr><tr><td>%s<td>%s<td>%s<td>%s<td>%s<td>%s</b>",
                  "#", lNick, lKills, lDeaths, lHs, lAcc )

	for (new i = 90; i < iMax && MAX_BUFFER_LENGTH - iLen > 90; i++)
	{
		if (equal(istate,"A")) copy(istate,3,"B")
                else copy(istate,3,"A")
                get_stats(i, izStats, izBody, t_sName, MAX_NAME_LENGTH)
...
format_topme(sBuffer[MAX_BUFFER_LENGTH + 1])
{
	...
	iLen = format( sBuffer, MAX_BUFFER_LENGTH,
                  "<head><META http-equiv=Content-Type content='text/html ;charset=UTF-8'></head><style>body{background:#191818;background-image: url(^"http://i.hizliresim.com/d59r4p.jpg^");margin:5pt;padding:2pt;font-family:Tahoma;color:#ccc}.A{background-color:#262626;opacity:0.6;filter:alpha(opacity=60);}.B{background-color:#0b0b0b;opacity:0.6;filter:alpha(opacity=60);}td{font-size:14px}</style><center><table width=600 background=^"http://i.hizliresim.com/v8GOr6.png^">" )
	iLen += format( sBuffer[iLen], MAX_BUFFER_LENGTH - iLen,
                  "<tr><td><br/> <td></tr><tr><td><br/> <td></tr><tr><td><br/> <td></tr><tr><td><br/> <td></tr><tr><td><br/> <b><td></tr><tr><td>%s<td>%s<td>%s<td>%s<td>%s<td>%s</b>",
                  "#", lNick, lKills, lDeaths, lHs, lAcc )

	for (new i = iRankPosDusuk; i < iMax && MAX_BUFFER_LENGTH - iLen > iRankPosDusuk; i++)
	{
		if (equal(istate,"A")) copy(istate,3,"B")
                else copy(istate,3,"A")
                get_stats(i, izStats, izBody, t_sName, MAX_NAME_LENGTH)
...
public cmdStatsMe(id)
{
	if (!SayStatsMe)
	{
		sayyaz(id, "!gcsplugin.com !y: Serverda o komut kapali")
		return PLUGIN_HANDLED
	}
...
public cmdTop15(id)
{
	...
	show_motd(id, g_sBuffer, "cs plugin.com En Iyi 10")

	return PLUGIN_CONTINUE
}
public cmdTop1020(id)
{
	if (!SayTop15)
	{
		sayyaz(id, "!gcsplugin.com !y: Serverda o komut kapali")
	...
	format_top1020(g_sBuffer)
	show_motd(id, g_sBuffer, "csplugin.com 10-20")

	return PLUGIN_CONTINUE
}
public cmdTop2030(id)
{
	if (!SayTop15)
	{
		sayyaz(id, "!gcsplugin.com !y: Serverda o komut kapali")
	...
	format_top2030(g_sBuffer)
	show_motd(id, g_sBuffer, "csplugin.com 20-30")

	return PLUGIN_CONTINUE
}
public cmdTop3040(id)
{
	if (!SayTop15)
	{
		sayyaz(id, "!gcsplugin.com !y: Serverda o komut kapali")
	...
	format_top3040(g_sBuffer)
	show_motd(id, g_sBuffer, "csplugin.com 30-40")

	return PLUGIN_CONTINUE
}
public cmdTop4050(id)
{
	if (!SayTop15)
	{
		sayyaz(id, "!gcsplugin.com !y: Serverda o komut kapali")
	...
	format_top4050(g_sBuffer)
	show_motd(id, g_sBuffer, "csplugin.com 40-50")

	return PLUGIN_CONTINUE
}
public cmdTop5060(id)
{
	if (!SayTop15)
	{
		sayyaz(id, "!gcsplugin.com !y: Serverda o komut kapali")
	...
	format_top5060(g_sBuffer)
	show_motd(id, g_sBuffer, "csplugin.com 50-60")

	return PLUGIN_CONTINUE
}
public cmdTop6070(id)
{
	if (!SayTop15)
	{
		sayyaz(id, "!gcsplugin.com !y: Serverda o komut kapali")
	...
	format_top6070(g_sBuffer)
	show_motd(id, g_sBuffer, "csplugin.com 60-70")

	return PLUGIN_CONTINUE
}
public cmdTop7080(id)
{
	if (!SayTop15)
	{
		sayyaz(id, "!gcsplugin.com !y: Serverda o komut kapali")
	...
	format_top7080(g_sBuffer)
	show_motd(id, g_sBuffer, "csplugin.com 70-80")

	return PLUGIN_CONTINUE
}
public cmdTop8090(id)
{
	if (!SayTop15)
	{
		sayyaz(id, "!gcsplugin.com !y: Serverda o komut kapali")
	...
	format_top8090(g_sBuffer)
	show_motd(id, g_sBuffer, "csplugin.com 80-90")

	return PLUGIN_CONTINUE
}
public cmdTop90100(id)
{
	if (!SayTop15)
	{
		sayyaz(id, "!gcsplugin.com !y: Serverda o komut kapali")
	...
	format_top90100(g_sBuffer)
	show_motd(id, g_sBuffer, "csplugin.com 90-100")
...
public cmdHp(id)
{
	...
	sayyaz(id, "!gcsplugin.com !y: %s", g_sBuffer)
...
public cmdRank(id)
{
	...
	//client_print(id, print_chat, "*Siralamadaki yeriniz %d kisi arasindan %d. %d oldurulen, %d vuruslar, %0.2f%% eff. ve %0.2f%% isabet"
	sayyaz(id, "!gcsplugin.com !y: Siralamadaki yeriniz; %d kisi arasindan %d. || %d Ã¶ldÃ¼rme, %d vuruslar, %0.2f%% eff. ve %0.2f%% isabet", iRankMax,iRankPos , izStats[STATS_KILLS], izStats[STATS_HITS], fEff, fAcc)
	return PLUGIN_CONTINUE
}

//Yer Söyle
public siralamadakiyerim(id){
	new iRankPos;
	new izStats[8], izBody[8]
	iRankPos = get_user_stats(id, izStats, izBody);
	iRankPosYuksek = iRankPos+5
	iRankPosDusuk = iRankPos-5
	if(iRankPos >= 1 && iRankPos <= 10){
		cmdTop15(id);
	}
	else {
		format_topme(g_sBuffer)
		show_motd(id, g_sBuffer, "csplugin.com Sizin Oldugunuz Sira")
	}
}
...
public end_game_stats()
{
	...
	else if (EndTop15)
	{
		...
		for (iPlayer = 0; iPlayer < iPlayers; iPlayer++)
		{
			...
			show_motd(iaPlayers[iPlayer], g_sBuffer, "Top 10")
		}
	}
